...
export const Route = createFileRoute("/create")({
  head: () => ({
    meta: [
      ...
      {
        property: "og:title",
        content: "Créer un événement — Quizz-App",
      },
      {
...
function CreatePage() {
  ...
  const handleTemplate = async () => {
    try {
      ...
      toast.error(
        "Le téléchargement du modèle a échoué. Réessayez.",
      );
    }
  };
  ...
  const handleFile = async (
    ...
    try {
      ...
      if (
        ...
        toast.error(
          "Aucune question trouvée dans ce fichier.",
        );
      }
    } catch {
      toast.error(
  ...
  if (created) {
    const presenterUrl =
      `${window.location.origin}/presenter/${created.adminToken}`;

    const participantUrl =
      `${window.location.origin}/e/${created.code}`;
    ...
    return (
      <div className="mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center px-6 py-16 text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          Partagez ce code ou scannez le QR Code
          pour rejoindre directement l'événement.
        </p>
    ...
    <div className="mx-auto min-h-screen max-w-3xl px-6 py-10">
      ...
      <div className="mt-8 rounded-3xl border border-dashed border-secondary/50 bg-secondary/5 p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Téléchargez le modèle, remplissez une
          ligne par question, puis importez le fichier.
        </p>
